...
Status: P0 source-of-truth matrix for v5.22.0.
...
| German pattern linter | `scripts/german_pattern_lint.py` | Cluster checks for patterns 2 (including advertising sections from pattern 44), 8, 13, 54, 58, 63, 64, 65 plus an `info` advisory candidate for pattern 72 | Deterministic cluster findings for selected naturalness signals; partial coverage only for patterns 2 and 44; candidate-only context prompt for pattern 72 | Most of the 72-pattern catalog; tricolons from pattern 9; general substance judgment for pattern 44; no confirmed pattern-72 verdict |
...
| Two-pass runner | `scripts/humanizer_two_pass.py` | Separate read-only audit and fresh candidate-bound rewrite with a frozen protection ledger | Host-enforced spans, structural scope, delete-only candidates, anchor checks, and Evidence gate | Complete fact/source verification or proof that every model-selected candidate is correct |
...
| `german_pattern_lint.py` | 2, 8, 13, 44, 54, 58, 63, 64, 65; 72 advisory | Pattern 2 includes advertising-figure clusters with pattern-44 section evidence; pattern 44 remains partial; pattern 72 stays a context-dependent `info` candidate |
...
## Late Catalog Pattern Coverage

| Pattern | Coverage | Notes |
|---:|---|---|
| 67 | Human reviewer / LLM judgment | Ankündigungs-Spaltsätze require context and cluster interpretation; no deterministic detector |
| 68 | Human reviewer / LLM judgment | Komparativ-Rahmung requires separating empty framing from a real, verifiable comparison |
| 69 | Human reviewer / LLM judgment | Register profiles and writing samples inform the judgment; no deterministic sentence-architecture detector |
| 70 | Human reviewer / LLM judgment | False agency requires semantic role and evidence checks; no deterministic detector |
| 71 | Human reviewer / LLM judgment | Neuigkeits- und Löschtest distinguish real precision from empty nuance; marker lint rejected |
| 72 | Advisory lint plus human reviewer / LLM judgment | Narrow `info` regex supplies a candidate only; counseling, quotation, and factual clarification require context |
...
- "The repo contains a 72-pattern catalog."
...
- "All 72 patterns are deterministically detected."
