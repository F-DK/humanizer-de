...
- **5.11.0** - M8 erkennt jetzt auch dichte Dokumentcluster aus „nicht A, sondern B“ und „A und nicht B“ als manuellen Prüffall. Der neue Befund fließt in die Preflight-Einschätzung ein, übernimmt für Zitate dieselben deutschen, Schweizer und englischen Zeichenpaare wie das Evidence-Gate, schützt auch teilweise überlappte Zitat- und Code-Spans und zählt verschachtelte Varianten nur einmal. Eindeutige Zahlkorrekturen mit Einheiten sowie Monat-Jahr-Angaben bleiben unbeanstandet. Schwellen und Katalogumfang ändern sich nicht.
...
- **5.10.1** - Auslieferungs-Patch. Wer den Skill als Plugin installiert hat, bekommt neue Fassungen erst, wenn sich die Versionsnummer ändert; neue Commits allein reichen nicht, weil die gepinnte Version den Cache gültig hält. Deshalb verlangt das [Marker-Aufnahmeprotokoll](docs/marker-aufnahmeprotokoll.md) ab jetzt für jede inhaltliche Änderung an ausgelieferten Dateien einen Patch-Bump. Die Version steht dazu nur noch in den beiden Plugin-Manifesten und nicht mehr zusätzlich im Marktplatz-Eintrag, weil ein veralteter Wert im Manifest den Marktplatz-Wert stumm überschreibt. Inhaltlich nachgezogen: Muster 26 kennt jetzt die `utm_source`-Varianten `openai` und `chatgpt`, und der Naturalness-Brief belegt die Grenzen automatischer Detektion mit der Expertenstudie von Russell, Karpinska und Iyyer.
...
- **5.8.0** - Zwei neue deterministische Prüfungen gegen Schreibmuster, die auch redigierte Texte häufig überstehen: Der Naturalness-Linter meldet jetzt negative Parallelismen (Muster 8 – gepaarte Verneinungen wie „kein X, kein Y“ oder „nicht X, nicht Y“) und übermäßige Inline-Fettschrift (Muster 13 – ab fünf Fett-Spannen je Dokument). Beide Schwellen sind an verifizierter menschlicher Prosa geeicht und lösen eine Warnung aus, keinen Blocker; „nicht X, sondern Y“-Sachkorrekturen sowie Fett in Code und Tabellen bleiben unberührt. Ergänzend operationalisieren drei neue Rule-Cards, wie sich diese Muster – und die bewusst urteilsbasierte Dichotomie-Zuspitzung (Muster 7), die als normales Deutsch kein Detektor wird – ohne Übertreibung entschärfen lassen.
