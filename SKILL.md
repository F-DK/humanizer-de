...
**Musterabdeckung je Pass.** Jeder Musterblock in [references/patterns.md](references/patterns.md) trägt eine Annotation `<!-- pass: N -->`. Die Pässe 1 bis 4 arbeiten jeweils **alle** Muster ihres Passes ab, nicht nur die unten namentlich genannten; die Nennungen sind Schwerpunkte, keine Liste. Fehlt ein Muster in Linter und Nennung, ist die Annotation sein einziger Weg in die Prüfung. Katalog laden, sobald ein Pass über seine Schwerpunkte hinausgeht.
...
**Audit-Zweig (Nutzer will Befunde, keine Redaktion).** Beim reinen Audit gilt kein Pass-Zuschnitt: [references/patterns.md](references/patterns.md) laden und **alle 72 Muster** durchgehen, auch die ohne Linter und ohne Nennung. Preflight `low` ist kein Grund, den Urteilsteil zu überspringen — er misst Oberflächencluster, keine rhetorischen Figuren. Cluster-Regel und Carve-outs gelten unverändert; Befunde werden benannt, nicht geändert.
